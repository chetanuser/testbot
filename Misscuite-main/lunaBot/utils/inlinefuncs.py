...
async def inline_help_func(__help__):
    ...
    answerss = [
        InlineQueryResultArticle(
            title="ɪɴʟɪɴᴇ ᴄᴏᴍᴍᴀɴᴅs𓆦",
            description="ʜᴇʟᴘ ʀᴇʟᴀᴛᴇᴅ ᴛᴏ ɪɴʟɪɴᴇ ᴜsᴀɢᴇ.",
            input_message_content=InputTextMessageContent(
                "ᴄʟɪᴄᴋ ᴀ ʙᴜᴛᴛᴏɴ ᴛᴏ ɢᴇᴛ sᴛᴀʀᴛᴇᴅ.😅"
            ),
            thumb_url="https://telegra.ph/file/ebd3f875fc52da59d0b64.jpg",
...
async def alive_function(answers):
    ...
    bot_state = "ᴅᴇᴀᴅ𓀿" if not await app.get_me() else "ᴀʟɪᴠᴇ✔"
    buttons.add(
        InlineKeyboardButton("ʏᴏʏᴏ-x", url=f"https://t.me/DEVU_ROBOT"),
        InlineKeyboardButton("ɢᴏ ɪɴʟɪɴᴇꪝ", switch_inline_query_current_chat=""),
    )

    msg = f"""
**[ᴅᴇᴠᴜ🧚‍♀️](https://t.me/DEVU_ROBOT):**
`ʙᴏᴛ-sᴛᴀᴛᴜs` : `{bot_state}`
`ᴜsᴇʀʙᴏᴛ` : `ᴊɪɴᴅᴀ ʜᴜ ᴠᴀɪ🥀`
`ᴘʏᴛʜᴏɴ` : `{pyver.split()[0]}`
`ᴘʏʀᴏɢʀᴀᴍ` : `{pyrover}`
`ᴍᴏɴɢᴏᴅʙ` : `{mongover}`
`ᴘʟᴀᴛᴇғᴏʀᴍ` : `{sys.platform}`
`sᴛᴀᴛᴜs` : [ʜᴜʜ ʙᴏᴛ](https://t.me/DEVU_ROBOT) | [ᴅᴇᴠᴇʟᴏᴘᴇʀ](https://t.me/HYPER_AD13)
"""
    answers.append(
        InlineQueryResultArticle(
            title="ᴀʟɪᴠᴇ🧚‍♀️",
            description="ᴄʜᴇᴄᴋ ᴅᴇᴠᴜ's sᴛᴀᴛs",
            thumb_url="https://telegra.ph/file/ebd3f875fc52da59d0b64.jpg",
            input_message_content=InputTextMessageContent(
...
async def about_function(answers):
    buttons = InlineKeyboard(row_width=2)
    bot_state = "ᴅᴇᴀᴅ" if not await app.get_me() else "ᴀʙᴏᴜᴛ"
    buttons.add(
        InlineKeyboardButton("[🇮🇳]ᴅᴇᴠ", url=f"https://t.me/SILENT_DEVS"),
        InlineKeyboardButton("ᴜᴘᴅᴀᴛᴇs", url=f"https://t.me/SILENT_BOTS"),
    )

    msg = f"""
[ᴅᴇᴠᴜ🧚‍♀️](https://t.me/lunatapibot) 
ʙᴜɪʟᴛ ᴡɪᴛʜ ᴜsɪɴɢ ᴘʏᴛʜᴏɴ-ᴛᴇʟᴇɢʀᴀᴍ-ʙᴏᴛ `ᴠ13.7` ʀᴜɴɴɪɴɢ ᴏɴ ᴘʏᴛʜᴏɴ `3.9.7`
...
async def translate_func(answers, lang, tex):
    ...
    msg = f"""
`ᴛʀᴀɴsʟᴀᴛᴇᴅ ꜰʀᴏᴍ {result.src} ᴛᴏ {result.dest}`
...
{result.translatedText}"""
    answers.extend(
        [
            InlineQueryResultArticle(
                title=f"ᴛʀᴀɴsʟᴀᴛᴇᴅ ғʀᴏᴍ {result.src} ᴛᴏ {result.dest}.",
...
`ᴅɪsᴄʀɪᴘᴛɪᴏɴ`: __{clean(i.definition)}__

`ᴇxᴀᴍᴘʟᴇ`: __{clean(i.example)}__"""
...
**ᴄʜᴀɴɴᴇʟ:** {i.channel}
**ᴅᴜʀᴀᴛɪᴏɴ:** {i.duration}
**ᴜᴘʟᴏᴀᴅᴇᴅ:** {i.publish_time}
**ᴅᴇsᴄʀɪᴘᴛɪᴏɴ:** {i.long_desc}"""
...
async def tg_search_func(answers, text, user_id):
    if user_id not in DEV_USERS:
        ...
        answers.append(
            InlineQueryResultArticle(
                title="ERROR",
                description="ᴏᴏᴘs ɪᴛs ᴏɴʟʏ ғᴏʀ ᴅᴇᴠs",
                input_message_content=InputTextMessageContent(msg),
            )
        )
        return answers
    if str(text)[-1] != ":":
        msg = "**ERROR**\n__ᴘᴜᴛ ᴀ ':' ᴀғᴛᴇʀ ᴛʜᴇ ᴛᴇxᴛ ᴛᴏ sᴇᴀʀᴄʜ__"
        answers.append(
            InlineQueryResultArticle(
                title="ERROR",
                description="ᴘᴜᴛ ᴀ ':' ᴀғᴛᴇʀ ᴛʜᴇ ᴛᴇxᴛ ᴛᴏ sᴇᴀʀᴄʜ",
                input_message_content=InputTextMessageContent(msg),
            )
        )
    ...
    async for message in app2.search_global(text, limit=49):
        ...
        caption = f"""
**ǫᴜᴇʀʏ:** {text}
**ɴᴀᴍᴇ:** {str(name)} [`{message.from_user.id}`]
**ᴄʜᴀᴛ:** {str(message.chat.title)} [`{message.chat.id}`]
**ᴅᴀᴛᴇ:** {ctime(message.date)}
**ᴛᴇxᴛ:** >>
...
async def music_inline_func(answers, query):
    ...
    group_invite = "https://t.me/SILENT_SUPPORT1"
    try:
        messages = [
            m

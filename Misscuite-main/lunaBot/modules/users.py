...
@run_async
def chat_checker(update: Update, context: CallbackContext, message):
    bot = context.bot
    try:
        if update.effective_message.chat.get_member(bot.id).can_send_messages is False:
            # message.repy_text("ʜᴜʜ, ɴᴏɴsᴇɴᴄᴇ ɢʀᴏᴜᴘ ɪ ᴀɴᴅ ᴍʏ ᴅᴇᴠs ᴅᴏɴ'ᴛ ᴀʟʟᴏᴡ ᴛᴏ sᴛᴀʏ ɪɴ ᴛʜɪs ᴄʜᴀᴛ ɢᴏᴏᴅ ʙʏ ᴅᴇᴍᴏɴs...")
...
def __user_info__(user_id):
    if user_id in [777000, 1087968824]:
        return """╘「<code>ɢʀᴏᴜᴘ ᴄᴏᴜɴᴛᴇᴅ</code>: <code>???</code> 」"""
    if user_id == dispatcher.bot.id:
        return """╘「<code>ᴄᴏᴜɴᴛᴇᴅ ɢʀᴏᴜᴘ</code>: <code>???</code> 」"""
    num_chats = sql.get_user_num_chats(user_id)
    return f"""╘「<code>ᴄᴏᴜɴᴛᴇᴅ ɢʀᴏᴜᴘ</code>: <code>{num_chats}</code> 」"""
